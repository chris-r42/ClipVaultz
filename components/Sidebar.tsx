...
export default function Sidebar({ isAdmin, currentUserId, members, className = '' }: SidebarProps) {
  ...
  return (
    <aside className={`w-60 shrink-0 h-screen sticky top-0 flex-col border-r border-[var(--card-border)] bg-[var(--card)] ${className}`}>
      {/* Logo */}
      <div className="px-4 h-14 flex items-center border-b border-[var(--card-border)] shrink-0">
      ...
      {/* Members list */}
      <div className="flex-1 overflow-y-auto px-3 py-3 border-t border-[var(--card-border)]">
        <MembersPresence members={members} currentUserId={currentUserId} />
      </div>

      {/* Sign out */}
      <div className="px-3 py-3 border-t border-[var(--card-border)] shrink-0">
        <SignOutButton />
      </div>
    </aside>
  )
